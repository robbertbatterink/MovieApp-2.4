...
from flask import render_template, request, jsonify, session as flaskSession ,redirect
...
def registreren():
	if request.method == 'POST':
            ...
            postBio = content['bio']
            if postNaam and postEmail and postPassword and postBio:
                    return registreer_gebruiker(postNaam, postEmail, postPassword, postBio)

            return "Alle verplichte velden invullen svp..."
		
		
def registreer_gebruiker(postNaam, postEmail, postPassword, postBio):
    gebruiker = user(user_name= postNaam, user_email= postEmail, user_password= postPassword, user_bio= postBio)
